...
final class TestBuffer: XCTestCase {
  ...
  func test_buffering_suspending() async {
    var gated = GatedSequence([1, 2, 3, 4, 5])
    let sequence = gated.buffer(limit: 5)
    var iterator = sequence.makeAsyncIterator()
...
//
  func test_bufferingOldest() async {
    validate {
      "X-12-   34-    5   |"
      $0.inputs[0].buffer(policy: .bufferingOldest(limit: 2))
      "X,,,[1,],,[2,],[3,][5,]|"
    }
  }
  ...
  func test_bufferingNewest() async {
    validate {
      "X-12-   34    -5|"
      $0.inputs[0].buffer(policy: .bufferingNewest(limit: 2))
      "X,,,[1,],,[3,],[4,][5,]|"
    }
  }
